...
class Phrase(object):
    def __init__(self, ID=None, speaker=None, utterance=[], mor=[], previous=None):
        self.ID = ID
...
#gets the utterance and mor for the given ID number
def getTiers(iter, givenID, previous):
    for elem in iter:
    	tiers = Phrase(givenID)
    	tiers.clear()
        #Each sentence has a speaker, a unique ID, and a mor tier
        tiers.speaker = "*" + elem.get("who") +":"
            ...
            for child in elem:
                ...
                if (child.tag == (ns + "w")):
                    ...
                    if child.get("type") == "fragment":             
                        tiers.mor.append("unk|" + phrase)
                        tiers.utterance.append(phrase)
                        continue
                    #is shortening
                    ...
                    if (child.find(".//%smwc/%spos/%sc" % (ns,ns,ns)) != None):
                        if (child[0].tail != None):
                            phrase = phrase + "+" + child[0].tail
                            tiers.mor.append(process_mor(child.getiterator(ns + "mor"), isCompound))
                    elif (phrase == "xxx" or phrase == "xx") :
                        tiers.mor.append("unk|" + phrase)
                    else:
                        tiers.mor.append(process_mor(child.getiterator(ns + "mor"), False))
                ...
                #is punctuation
                    ...
                    tiers.utterance.append(d[punctuation])
                    tiers.mor.append(d[punctuation])
                
                #is a central comma
                if (child.tag == (ns + "s")):
                    if child.get("type") == "comma":
                        tiers.utterance.append(",")
                        tiers.mor.append(",")
