...
def parseFile(filename, errors):
    ...
    finished = speaker(iter, filename, errorFile, int(startLine))
...
def speaker(iter, filename, errorFile, startLine):
	previous = Phrase(startLine-1)
	if startLine == 0:
		previous = None
	else:
		previous = getTiers(iter, startLine-1, None)
	current = getTiers(iter, startLine, previous)
	while current.mor != None:
		check(current, errorFile, filename)
		startLine = startLine + 1
		current = getTiers(iter, startLine, current)
	else:
		return 1
	
def check(current, errorFile, filename):
	current.printSentence()
	error = raw_input("Error? Type Y or N, followed by Enter\n")
	if error == "y" or error == "Y":
		print "-------------------------------------------------------------"
		processError(current, 0, errorFile, filename)
		print "-------------------------------------------------------------"
		...
		if current.ID < 1:
			...
			check(current, errorFile, filename)
		else: check(current.last, errorFile, filename)
	else:
		print "\nCommand not recognized:"
		check(current, errorFile, filename)
		
def processError(tiers, i, errorFile, filename):
    ...
    while i < length:
    	if i < 0:
    		...
    		if tiers.ID < 1:
    			print "\nHEY! You're already at the first line of the file!!\n" 
    			check(tiers, errorFile, filename)
    		else: check(tiers.last, errorFile, filename)
     	...
     	if tiers.utterance[i] != "," or tiers.mor[i] != ",":
     		tiers.printWord(i)
     	 	...
     	 	if error == "y" or error == "Y":
				...
				processError(tiers, i, errorFile, filename)
				break
